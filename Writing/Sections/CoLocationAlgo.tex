...
\Fn{\FSingle{seeker priorities, owner priorities, owner slates, seeker indicators}}{
 \texttt{Find a job for a seeker $c \in A_{c_e}$ only if they are not slated for a job} \;
 \texttt{Iterate down the list of seeker $c$'s preferences until they are places in a job} \;
 $p = 1$\;
 \While{$I_{c_e} == 0$}{
...
\Fn{\FCouple{seeker priorities, owner priorities, owner slates, seeker indicators}}{
 \texttt{Find a job for the seeking couple $c_e = (c_1, c_2)$ if either of them are not slated for a job} \;
\texttt{While both members of the seeking entity do not have a job, iterate down the list of preferences of seeker entity $c_e$} \;
 Initialize $p = 1$\;
 \While{$I_{c_e} == 0$}{
