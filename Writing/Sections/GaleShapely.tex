...
\KwResult{There are no pairs $(O_i, S_i), (O_j, S_j)$ such that the pairings $(O_i, S_j), (O_j, S_i)$ would be preferred by all parties.} 
 ...
 \For{$c \in \{1, \dots, m\}$}{
 \texttt{Find a job for a seeker $c$ only if they are not slated for a job} \;
 \If{$I_c == 0$}{
 \texttt{Look for the job that is seeker $c$'s $g^{th}$ preference} \;
 $j = r$ such that $P^S_{r,c} = g_c$ \;
 \texttt{If job $j$ has open positions} \;
  \uIf{$|O_j| < l_j$}{
   \texttt{Add the seeker $c$ to the slate of owner of job $j$} \;
   $O_j += S_c$\;
   \texttt{Indicate that seeker $c$ tentatively has a job} \;
   $I_c = 1$\;
   }
   \texttt{Or if seeker $c$ preferred by job owner $j$ than their least preferred person currently on their slate} \;
 ...
 \texttt{Indicate that the $g^{th}+1$ preference of seeker $c$ has been considered} \; 
 $g_c++$\;
 } 
 }
 }
